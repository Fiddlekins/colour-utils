...
const parseCss_1 = require("../src/parseCss");
describe('parseColourChannelString', () => {
    it('should parse "X" correctly', () => {
        ...
        for (const [input, output] of tests) {
            (0, chai_1.expect)((0, parseCss_1.parseColourChannelString)(input)).to.deep.equal(output, `for input ${input}`);
        }
    });
    it('should parse "X%" correctly', () => {
        const tests = [
...
describe('parseAlphaChannelString', () => {
    it('should parse "X" correctly', () => {
        ...
        for (const [input, output] of tests) {
            (0, chai_1.expect)((0, parseCss_1.parseAlphaChannelString)(input)).to.deep.equal(output, `for input ${input}`);
        }
    });
    it('should parse "X%" correctly', () => {
        const tests = [
...
describe('parseAngle', () => {
    it('should parse "X" correctly', () => {
        ...
        for (const [input, output] of tests) {
            (0, chai_1.expect)((0, parseCss_1.parseAngle)(input)).to.deep.equal(output, `for input ${input}`);
        }
    });
    it('should parse "Xdeg" correctly', () => {
        ...
        for (const [input, output] of tests) {
            (0, chai_1.expect)((0, parseCss_1.parseAngle)(input, 'deg')).to.deep.equal(output, `for input ${input}`);
        }
    });
    it('should parse "Xrad" correctly', () => {
        ...
        for (const [input, output] of tests) {
            (0, chai_1.expect)((0, parseCss_1.parseAngle)(input, 'rad')).to.deep.equal(output, `for input ${input}`);
        }
    });
    it('should parse "Xgrad" correctly', () => {
        ...
        for (const [input, output] of tests) {
            (0, chai_1.expect)((0, parseCss_1.parseAngle)(input, 'grad')).to.deep.equal(output, `for input ${input}`);
        }
    });
    it('should parse "Xturn" correctly', () => {
        ...
        for (const [input, output] of tests) {
            (0, chai_1.expect)((0, parseCss_1.parseAngle)(input, 'turn')).to.deep.equal(output, `for input ${input}`);
        }
    });
});
describe('parseHex', () => {
...
describe('parseCssToRgb', () => {
    it('should parse "rgb(R G B)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToRgb)('rgb(0 0 0)')).to.deep.equal({ r: 0, g: 0, b: 0, a: 1 });
    });
    it('should parse "rgb(R, G, B)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToRgb)('rgb(0, 0, 0)')).to.deep.equal({ r: 0, g: 0, b: 0, a: 1 });
    });
    it('should parse "rgba(R G B / A)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToRgb)('rgba(0 0 0 / 0)')).to.deep.equal({ r: 0, g: 0, b: 0, a: 0 });
    });
    it('should parse "rgba(R, G, B, A)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToRgb)('rgba(0, 0, 0, 0)')).to.deep.equal({ r: 0, g: 0, b: 0, a: 0 });
    });
...
describe('parseCssToHsl', () => {
    it('should parse "hsl(H S L)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToHsl)('hsl(0 0% 0%)')).to.deep.equal({ h: 0, s: 0, l: 0, a: 1 });
    });
    it('should parse "hsl(H, S, L)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToHsl)('hsl(0, 0%, 0%)')).to.deep.equal({ h: 0, s: 0, l: 0, a: 1 });
    });
    it('should parse "hsla(H S L / A)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToHsl)('hsla(0 0% 0% / 0)')).to.deep.equal({ h: 0, s: 0, l: 0, a: 0 });
    });
    it('should parse "hsla(H, S, L, A)" correctly', () => {
        (0, chai_1.expect)((0, parseCss_1.parseCssToHsl)('hsla(0, 0%, 0%, 0)')).to.deep.equal({ h: 0, s: 0, l: 0, a: 0 });
    });
});
//# sourceMappingURL=parseCss.test.js.map